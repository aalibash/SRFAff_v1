function model = srfAffTrain_CAD120( varargin )
...
dfs={'imWidth',32, 'gtWidth',16, 'nPos',50000, 'nNeg',50000, ...
  ...
  'hogFeatDir','../hmp/icra_2015_results/hog/','tools_names','','posSkip', 1, 'negSkip', 1};
...
if opts.treeTrainID > 0
    ...
    tools_names=opts.tools_names;
    toolsIds = dlmread('tool_type_CAD120.txt'); % list of tools with their main affordance types
...
for pp=1:length(toolsSet)
    ...
    for gg=1:length(gtN)
        if ~opts.bCleanDepth
            imgIds=dir([gtD, gtN{gg}  '/*RGB*.png']); imgIds={imgIds.name};
            imgIds=strcat([gtD, gtN{gg} '/'], imgIds)';
            img_fp=[img_fp ;imgIds];
        else
            % precomputed data does not exist
        end
        imgIds=dir([gtD, gtN{gg}  '/*Depth*.png']); imgIds={imgIds.name};
        imgIds=strcat(opts.vggFeatDir, imgIds)'; % use cleaned depth
